...
<style>
    ...
    <body>
        <div>
            ...
            {{ strtoupper($customer->firstname.' '.$customer->middle_name.' '.$customer->surname) }}<br><br>
            {{ strtoupper($customer->address) }}<br>
            {{ strtoupper($customer->landmark.', '.$customer->state) }}<br><br><br>
            Sir,<br>
            OFFER OF N{{ number_format($offer->amount_recommended,2) }} LOAN FACILITY<br><br>
            The management of SWISS CREDIT LIMITED (the <q>Company</q>) has approved an offer of a loan facility to you {{ strtoupper($customer->firstname.' '.$customer->middle_name.' '.$customer->surname) }} (<q>The Borrower</q>) under the following terms and conditions:<br><br>
            ...
            <table class="summ">
                ...
                <tr>
                    <td>FACILITY</td>
                    <td>{{ ucwords($customer->purpose) }}</td>
                </tr>
                <tr>
                    <td>LOAN AMOUNT</td>
                    <td>N{{ number_format($offer->amount_recommended,2) }}</td>
                </tr>
                @if($offer->outstanding_bal > 0)
                <tr>
                    <td>OUTSTANDING BALANCE</td>
                    <td>N{{ number_format($offer->outstanding_bal,2) }}</td>
                </tr>
                @endif
                <tr>
                    <td>TENURE</td>
                ...
                <tr>
                    <td>INTEREST RATE</td>
                    <td>{{ $offer->interest }}% PER MONTH</td>
                </tr>
                <tr>
            ...
            I <b><span class="ddotted"> {{ strtoupper($customer->firstname.' '.$customer->middle_name.' '.$customer->surname) }}</span></b> hereby, unconditionally as i have read and understood the terms with total clarity.<br><br><br>
                ...
                <ol class="number">
                    ...
                    <li><div class="title">Events of defaults</div>
                        ...
                        <ol class="alpha">
                            ...
                            <li>If the Borrower defaults in the performance or the observance of any term, condition or covenant herein and such breach or default continues un-remedied after fourteen day’s notice has been given.</li>
                            <li>If the Borrower’s creditors obtain a court order or injunction against the Borrower’s accounts</li>
                            <li>Default on the interest payment upon falling due would attract compound interest of {{ $offer->default_charge }}% flaton total outstanding debt; inclusive of principal, loan interest and accrued default charges</li>
                            ...
                            <li><b>Swiss Credit reserves the right to use every legal means possible to recover this loan, which include the use of third party.</b></li>
                        </ol>
                    </li>
            ...
            <div><br>LETTER OF ACKNOWLEDGEMENT<br><br>
                @if(isset($signature) && $signature->inuse != null)
                I <b><span class="ddotted"> {{ strtoupper($customer->firstname.' '.$customer->middle_name.' '.$customer->surname) }}</span></b> a staff of <b><span class="ddotted"> {{ strtoupper($customer->current_employer) }}</span></b> Hereby undertake to pay the monthly rental as captured beow until i fully liquidate my indebtedness, failing which I authorize Swiss Credit Limited to inform my employers and the payment of the debt should be deducted from my salary forthwith.<br><br>
                @else
                    ...
                    <tr>
                        ...
                        <td>
                            @if($i == 0)
                                 ...
                                 @else
                                  {{ Utils::getRepayDate($offer->pay_date, Utils::getTenorLastDay(1-$deductMonth, $offer->updated_at)) }}-{{ date('M', strtotime(Utils::getTenorLastDay(1-$deductMonth, $offer->updated_at))) }}-{{ date('Y', strtotime(Utils::getTenorLastDay(1, $offer->updated_at))) }}
                                 @endif
                            @else
                            {{ Utils::getRepayDate($offer->pay_date, Utils::getTenorLastDay($i-$deductMonth, $offer->updated_at)) }}-{{ ucwords(date('M', strtotime(Utils::getTenorLastDay($i+1-$deductMonth, $offer->updated_at)))) }}-
                            @if($i > 0)
                                @if(ucwords(date('M', strtotime(Utils::getTenorLastDay($i+1-$deductMonth, $offer->updated_at)))) == 'Jan')
                                        {{ date('Y', strtotime(Utils::getTenorLastDay($i+1-$deductMonth, $offer->updated_at))) }}
                                @else
                                {{ date('Y', strtotime(Utils::getTenorLastDay($i, $offer->updated_at))) }}
                                @endif
                            @else
